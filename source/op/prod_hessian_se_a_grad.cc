...
REGISTER_OP("ProdHessianSeAGrad")
.Attr("T: {float, double}")
.Input("grad: T")
.Input("net_deriv: T")
.Input("in_deriv: T")
...
class ProdHessianSeAGradOp : public OpKernel 
{
public:
  explicit ProdHessianSeAGradOp(OpKernelConstruction* context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(                                                                  \
    Name("ProdHessianSeAGrad").Device(DEVICE_CPU).TypeConstraint<T>("T"),                      \
    ProdHessianSeAGradOp<CPUDevice, T>); 
REGISTER_CPU(float);
REGISTER_CPU(double);
